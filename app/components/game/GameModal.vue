...
<template>
  <UModal v-model="isOpen">
    <UCard 
      class="bg-cyber-900 overflow-hidden border-4"
      :class="type === 'win' ? 'border-neon-cyan' : 'border-neon-magenta'"
    >
      <template #header>
        <div 
          v-if="type === 'win'" 
          class="flex items-center gap-4 p-4 bg-cyan-500/10"
        >
        ...
        <div 
          v-else 
          class="flex items-center gap-4 p-4 bg-magenta-500/10"
        >
      ...
      <template #body>
        <div class="p-6 text-center">
          <p v-if="type === 'win'" class="text-xl text-cyber-200 font-mono italic">
            ...
            <span v-else class="text-sm text-magenta-400 mt-4 block">PROTOCOLE FINAL TERMINÉ. VOUS ÊTES UN MAÎTRE ! 🏆</span>
          </p>
          <p v-else class="text-xl text-cyber-200 font-mono italic">
            Erreur logique détectée.
            <br>
            <span class="text-sm text-magenta-400 opacity-80 mt-4 block font-black uppercase">Vérifiez vos séquences d'instructions !</span>
          </p>
        </div>
      </template>
      ...
      <template #footer>
        <div class="flex justify-center gap-4 pb-4">
          <UButton
            v-if="type === 'win' && hasNextLevel"
            icon="i-ph-arrow-right-bold"
            color="primary"
            size="xl"
            class="glow-cyan font-black uppercase tracking-widest px-8"
          ...
          <UButton
            ...
            color="secondary"
            size="xl"
            class="glow-magenta font-black uppercase tracking-widest px-8"
            @click="handleRetry"
          >
            Réessayer
          </UButton>
