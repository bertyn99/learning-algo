...
import type { Level, Robot, Position } from '~/types/game'
...
const robotStyle = computed(() => {
  ...
  const x = props.robot.x * cellSize
  const y = props.robot.y * cellSize
  ...
  return {
    ...
    left: `${x}%`,
    top: `${y}%`,
    transform: `rotate(${rotationMap[props.robot.dir]}deg)`,
  ...
  >
    ...
    <div class="grid w-full h-full relative z-10 p-1 gap-1" :style="gridStyle">
      <div
        ...
        :key="index"
        class="aspect-square border-4 rounded-md relative transition-all duration-500"
        :class="[
      ...
      >
        ...
        <div v-if="isGoalCell(index)" class="absolute inset-0 flex items-center justify-center overflow-hidden">
          ...
          <div class="absolute inset-0 bg-linear-to-br from-yellow-300/50 to-yellow-500/50" />

          <!-- Animated rotating ring -->
          <div class="absolute inset-3 border-4 border-yellow-200 rounded-full animate-spin-slow opacity-70" />

          <!-- Star burst lines -->
          <div class="absolute w-full h-px bg-yellow-200" />
          <div class="absolute h-full w-px bg-yellow-200" />
          <div class="absolute w-full h-px bg-yellow-200 rotate-45 origin-center" />
          <div class="absolute w-full h-px bg-yellow-200 -rotate-45 origin-center" />
      ...
      <div class="absolute z-20 pointer-events-none flex items-center justify-center p-[2px]" :style="robotStyle">
        <div class="relative w-full h-full flex items-center justify-center">
          ...
          <div class="absolute inset-0 bg-primary/20 blur-xl rounded-full animate-pulse-slow" />
